...
export default function ProfilePage() {
  ...
  useEffect(() => {
    // Fetch session to know if the viewer owns this profile
  ...
  useEffect(() => {
    fetch("/api/users/profile", { cache: "no-store" })
      .then(async (res) => {
        ...
        // Redirect if the session user's ID doesn't match the route param
        if (data.user.id !== id) {
  ...
  return (
    <EnliveShell title="Profile" hideHeroHeader>
      <main className="mx-auto w-full max-w-2xl space-y-4">
        ...
        >
          ...
          <div className="px-6 pb-5">
            <div className="flex items-end justify-between" style={{ marginTop: "-42px" }}>
              ...
              {/* Edit / Cancel — only shown to the owner */}
              {isOwner && !editing && (
                <button
                  type="button"
        ...
        {/* ── Edit form (owner only, when editing) ── */}
        {isOwner && editing && (
          <div
